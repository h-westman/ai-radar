...
def url(radar_id, practice_id):
    return f"/api/radars/{radar_id}/notes/{practice_id}"
...
def test_create_then_update_note(client, session, ids):
    ...
    revisions = revisions_for(session, "radar_note", f"{ids[0]}:{ids[1]}")
...
def test_unknown_radar_or_practice_is_404(client, ids):
    radar_id, practice_id = ids
    assert (
        client.put(url(999999, practice_id), json={"version": 0, "body_md": ""}).status_code == 404
    )
    assert client.put(url(radar_id, 999999), json={"version": 0, "body_md": ""}).status_code == 404
