...
def place(radar, practice, adoption, value, eff, removed=False):
    return PlacementRow(
        id=next(_ids),
        radar_id=radar,
...
def test_radar_frame_lists_on_radar_practices_only():
    rows = [
        place(1, Y, 30, 40, JAN),
        place(1, X, 70, 80, JAN),
...
def test_radar_frame_skips_practices_not_in_practice_ids():
...
def test_org_frame_counts_non_users_as_zero_adoption():
    ...
    assert org_frame(radars(1, 2, 3), rows, ALL, JAN) == [
        Point(
            practice_id=X,
...
def test_radars_count_only_once_they_have_started():
    rows = [place(1, X, 80, 90, JAN), place(2, X, 40, 70, JAN), place(3, Y, 20, 20, MAR)]
    assert active_radar_ids(radars(1, 2, 3), rows, FEB) == {1, 2}
    [x] = org_frame(radars(1, 2, 3), rows, ALL, FEB)
    assert x.adoption == 60  # (80 + 40) / 2


def test_backdated_radar_counts_from_its_backdated_date():
    rows = [place(1, X, 80, 90, JAN)]
    assert active_radar_ids(radars(1), rows, JAN) == {1}


def test_archived_radars_drop_out_from_archive_date():
    archived_mid_feb = datetime(2026, 2, 15, tzinfo=UTC)
    rows = [place(1, X, 80, 90, JAN), place(2, X, 40, 70, JAN)]
    roster = radars(1, 2, archived={2: archived_mid_feb})
...
def test_removed_practice_keeps_radar_active_as_zero():
    rows = [
        place(1, X, 80, 90, JAN),
        place(2, X, 40, 70, JAN),
        place(2, X, 40, 70, FEB, removed=True),
    ]
    [x] = org_frame(radars(1, 2), rows, ALL, FEB)
    assert (x.adoption, x.value, x.radars) == (40, 90, 1)


def test_rounding_is_half_up():
    rows = [place(1, X, 45, 50, JAN), place(2, Y, 50, 50, JAN)]
    x = next(p for p in org_frame(radars(1, 2), rows, ALL, JAN) if p.practice_id == X)
    assert x.adoption == 23  # 22.5 rounds up


def test_org_frame_is_empty_without_active_radars():
    assert org_frame(radars(1), [], ALL, JAN) == []


def test_build_frames_dispatches_on_scope():
    rows = [place(1, X, 80, 90, JAN), place(2, X, 40, 70, FEB)]
    org = build_frames(
        scope_radar_id=None, radars=radars(1, 2), placements=rows, practice_ids=ALL, dates=[JAN, FEB]
    )
    assert [f.date for f in org] == [JAN, FEB]
    assert org[0].points[0].adoption == 80  # only radar 1 active in January
    assert org[1].points[0].adoption == 60
    radar = build_frames(
        scope_radar_id=2, radars=radars(1, 2), placements=rows, practice_ids=ALL, dates=[JAN, FEB]
    )
    assert radar[0].points == []
    assert radar[1].points == [Point(practice_id=X, adoption=40, value=70, radars=1)]


def test_point_tracks_radar_count_and_positions():
